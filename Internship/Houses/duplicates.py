...
houses = [102,110,113,115,116,117,118,120]

for u in range (len(houses)):
		for q in range (len(houses)):
			if(u!=q):
				current_input_directory = 'datasets/hh%d/hh%d_hh%d' %(houses[u],houses[u],houses[q])
				input1 = '%s/prob_source.csv' %(current_input_directory)
				input2 = '%s/prob_target.csv' %(current_input_directory)
				input3 = '%s/tree_path_vectors_source.csv' %(current_input_directory)
				input4 = '%s/tree_path_vectors_target.csv' %(current_input_directory)
				tppf = open(current_input_directory + '/source_noDup_prob.csv','w')
				tppf1 = open(current_input_directory + '/source_noDup_feature_vec.csv','w')

				tpdf = open(current_input_directory + '/target_noDup_prob.csv','w')
				tpdf1 = open(current_input_directory + '/target_feature_vec.csv','w')
